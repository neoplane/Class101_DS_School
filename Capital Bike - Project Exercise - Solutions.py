...
# and bike return is automated via a network of kiosk locations throughout a city. Using these systems, people are
# able rent a bike from a one location and return it to a different place on an as-needed basis. Currently,
...
# function as a sensor network, which can be used for studying mobility in a city. In this competition, participants
# are asked to combine historical usage patterns with weather data in order to forecast bike rental demand in the
# Capital Bikeshare program in Washington, D.C.
# 
# 
# ___

# ** Import Apache Spark 2.1libraries and setup environment variables. In case for cluster deployment you should
# probably remove this cell. **
...
SPARK_PATH = '/usr/local/Cellar/apache-spark/2.4.4/libexec'
...
# ___
# 
# We are provided hourly rental data spanning two years. For this competition, the training set is comprised of the
# first 19 days of each month, while the test set is the 20th to the end of the month. We must predict the total
# count of bikes rented during each hour covered by the test set, using only information available prior to the
# rental period.
# 
# ### Data Fields:
# 
...
# ** We will try both methods: predict amount of casual and regitered rentals separately and predict sum of those (
# count) **
...
def set_hour_cat(dataset):
    ...
    customers_rental.loc[((customers_rental['hour'] == 6) | (customers_rental['hour'] == 10) | (
            customers_rental['hour'] == 11) | (customers_rental['hour'] == 22) | (
                                  customers_rental['hour'] == 23)) & (
                                 customers_rental['workingday'] == 1), 'hour_cat'] = 1
    customers_rental.loc[(customers_rental['hour'] >= 7) & (customers_rental['hour'] <= 21) & (
            customers_rental['workingday'] == 1), 'hour_cat'] = 2
    customers_rental.loc[
        ((customers_rental['hour'] == 8) | (customers_rental['hour'] == 17) | (customers_rental['hour'] == 18)) & (
                customers_rental['workingday'] == 1), 'hour_cat'] = 3

    # workingday = 0
    customers_rental.loc[
        ((customers_rental['hour'] >= 22) | (customers_rental['hour'] <= 1) | (customers_rental['hour'] == 8)) & (
                customers_rental['workingday'] == 0), 'hour_cat'] = 4
    customers_rental.loc[((customers_rental['hour'] >= 2) & (customers_rental['hour'] <= 7)) & (
            customers_rental['workingday'] == 0), 'hour_cat'] = 5
    customers_rental.loc[
        ((customers_rental['hour'] == 9) | (customers_rental['hour'] == 20) | (customers_rental['hour'] == 21)) & (
                customers_rental['workingday'] == 0), 'hour_cat'] = 6
...
def set_independent_weather(dataset):
    ...
    customers_rental.loc[(customers_rental['month'] == 12) & (
            (customers_rental['day'] == 24) | (customers_rental['day'] == 26)), "workingday"] = 0
    customers_rental.loc[(customers_rental['month'] == 12) & (
            (customers_rental['day'] == 24) | (customers_rental['day'] == 25) | (
    ...
    customers_rental.loc[(customers_rental['year'] == 2011) & (customers_rental['month'] == 1) & (
            customers_rental['day'] == 26), "holiday"] = 1
    customers_rental.loc[
        (customers_rental['year'] == 2011) & (customers_rental['month'] == 9) & (customers_rental['day'] >= 4) & (
                customers_rental['day'] <= 8), "holiday"] = 1
    customers_rental.loc[(customers_rental['year'] == 2011) & (customers_rental['month'] == 8) & (
            customers_rental['day'] == 28), "holiday"] = 1
    customers_rental.loc[
        (customers_rental['year'] == 2011) & (customers_rental['month'] == 4) & (customers_rental['day'] >= 27) & (
                customers_rental['day'] <= 28), "holiday"] = 1
    # 2012
...
def prepare_data(dataset):
    ...
    for hour_cat in hour_cat_list:
        ...
        for col in dependent_variables:
            ...
            bc = boxcox(customers_rental[(customers_rental['hour_cat'] == hour_cat) & (customers_rental['train'] == 1)][
                            col] + 1)

            # bc[0] has transformed data, bc[1] has used lambda
            customers_rental.loc[
                (customers_rental['hour_cat'] == hour_cat) & (customers_rental['train'] == 1), '%s_bc' % col] = bc[0]

            customers_rental.loc[
                (customers_rental['hour_cat'] == hour_cat) & (customers_rental['train'] == 1), '%s_cat' % col] = pd.cut(
                customers_rental[(customers_rental['hour_cat'] == hour_cat) & (customers_rental['train'] == 1)][
                    '%s_bc' % col], bins=64, labels=range(0, 64), include_lowest=True)
...
for label in dependent_variables:
    ...
    for col in range(0, 8):
        sns.distplot(all_no_dummy[(all_no_dummy['hour_cat'] == col) & (all_no_dummy['train'] == 1)][label], bins=24,
                     ax=axes[(col // 4)][(col % 4)], axlabel=label + ', HOUR_CAT = ' + str(col), color=color)
...
for col in dependent_variables_bc:
    ...
    for hour_cat in hour_cat_list:
        print("Hour_cat " + str(hour_cat) + ": " + str(
            kstest(all_no_dummy[(all_no_dummy['hour_cat'] == hour_cat) & (all_no_dummy['train'] == 1)][col], 'norm')))
    print()
...
for col in dependent_variables:
    ...
    for hour_cat in hour_cat_list:
        print("Hour_cat " + str(hour_cat) + ": " + str(
            skew(all_no_dummy[(all_no_dummy['hour_cat'] == hour_cat) & (all_no_dummy['train'] == 1)]['%s_bc' % col])))
    print()
...
for col in dependent_variables:
    ...
    for hour_cat in hour_cat_list:
        print("Hour_cat " + str(hour_cat) + ": " + str(
            skew(all_no_dummy[(all_no_dummy['hour_cat'] == hour_cat) & (all_no_dummy['train'] == 1)][col])))
    print()
...
monthAggregated = pd.DataFrame(
    all_no_dummy[all_no_dummy['train'] == 1].groupby("month_cat")["count"].mean()).reset_index()
...
hourAggregated = pd.DataFrame(
    all_no_dummy[all_no_dummy['train'] == 1].groupby(["hour", "season_cat"], sort=True)["count"].mean()).reset_index()
sns.pointplot(x=hourAggregated["hour"], y=hourAggregated["count"], hue=hourAggregated["season_cat"],
...
hourAggregated = pd.DataFrame(all_no_dummy[all_no_dummy['train'] == 1].groupby(["hour", "dayofweek_cat"], sort=True)[
                                  "count"].mean()).reset_index()
sns.pointplot(x=hourAggregated["hour"], y=hourAggregated["count"], hue=hourAggregated["dayofweek_cat"],
...
hourTransformed = pd.melt(all_no_dummy[all_no_dummy['train'] == 1][["hour", "casual", "registered"]], id_vars=['hour'],
...
sns.pairplot(all_no_dummy_interpolate[all_no_dummy_interpolate['train'] == 1],
             vars=continuous_var + dependent_variables, hue='holiday', palette='coolwarm')

# ** There is some correlation (showed on some next cells on heatmap) between casual and registred users rentals. **

# In[56]:

sns.pairplot(all_no_dummy_interpolate[all_no_dummy_interpolate['train'] == 1], vars=['casual', 'registered'],
             palette='coolwarm')
...
sns.distplot(all_no_dummy[all_no_dummy['train'] == 1]['temp'], bins=24, ax=axes[0][0], axlabel='TRAIN: temp',
             color='orange')
sns.distplot(all_no_dummy_interpolate[all_no_dummy_interpolate['train'] == 1]['temp'], bins=24, ax=axes[1][0],
             axlabel='TRAIN: temp_interpolate', color='orange')
sns.distplot(all_no_dummy[all_no_dummy['train'] != 1]['temp'], bins=24, ax=axes[2][0], axlabel='TEST: temp',
             color='orange')
sns.distplot(all_no_dummy_interpolate[all_no_dummy_interpolate['train'] != 1]['temp'], bins=24, ax=axes[3][0],
             axlabel='TEST: temp_interpolate', color='orange')

sns.distplot(all_no_dummy[all_no_dummy['train'] == 1]['atemp'], bins=24, ax=axes[0][1], axlabel='TRAIN: atemp',
             color='red')
sns.distplot(all_no_dummy_interpolate[all_no_dummy_interpolate['train'] == 1]['atemp'], bins=24, ax=axes[1][1],
             axlabel='TRAIN: atemp_interpolate', color='red')
sns.distplot(all_no_dummy[all_no_dummy['train'] != 1]['atemp'], bins=24, ax=axes[2][1], axlabel='TEST: atemp',
             color='red')
sns.distplot(all_no_dummy_interpolate[all_no_dummy_interpolate['train'] != 1]['atemp'], bins=24, ax=axes[3][1],
             axlabel='TEST: atemp_interpolate', color='red')

sns.distplot(all_no_dummy[all_no_dummy['train'] == 1]['humidity'], bins=24, ax=axes[0][2], axlabel='TRAIN: humidity',
             color='green')
sns.distplot(all_no_dummy_interpolate[all_no_dummy_interpolate['train'] == 1]['humidity'], bins=24, ax=axes[1][2],
             axlabel='TRAIN: humidity_interpolate', color='green')
sns.distplot(all_no_dummy[all_no_dummy['train'] != 1]['humidity'], bins=24, ax=axes[2][2], axlabel='TEST: humidity',
             color='green')
sns.distplot(all_no_dummy_interpolate[all_no_dummy_interpolate['train'] != 1]['humidity'], bins=24, ax=axes[3][2],
             axlabel='TEST: humidity_interpolate', color='green')

sns.distplot(all_no_dummy[all_no_dummy['train'] == 1]['windspeed'], bins=24, ax=axes[0][3], axlabel='TRAIN: windspeed',
             color='blue')
sns.distplot(all_no_dummy_interpolate[all_no_dummy_interpolate['train'] == 1]['windspeed'], bins=24, ax=axes[1][3],
             axlabel='TRAIN: windspeed_interpolate', color='blue')
sns.distplot(all_no_dummy[all_no_dummy['train'] != 1]['windspeed'], bins=24, ax=axes[2][3], axlabel='TEST: windspeed',
             color='blue')
sns.distplot(all_no_dummy_interpolate[all_no_dummy_interpolate['train'] != 1]['windspeed'], bins=24, ax=axes[3][3],
             axlabel='TEST: windspeed_interpolate', color='blue')
...
sns.heatmap(all_no_dummy_interpolate[all_no_dummy_interpolate['train'] == 1].drop(
    dependent_variables + dependent_variables_cat + dependent_variables_log, axis=1).corr(), annot=False,
            cmap='coolwarm')
plt.tight_layout
...
train_no_dummy_year_piv = all_no_dummy_interpolate[all_no_dummy_interpolate['train'] == 1].pivot_table(values='count',
                                                                                                       index='month',
                                                                                                       columns='year').rename(
    index=monthDict)
...
train_no_dummy_month_piv = all_no_dummy_interpolate[all_no_dummy_interpolate['train'] == 1].pivot_table(values='count',
                                                                                                        index='hour',
                                                                                                        columns='month').rename(
    columns=monthDict)
...
train_no_dummy_workingday_piv = all_no_dummy_interpolate[all_no_dummy_interpolate['train'] == 1].pivot_table(
    values='count', index='hour', columns=['workingday'])
...
all_no_dummy_interpolate[
    (all_no_dummy_interpolate['train'] == 1) & (all_no_dummy_interpolate['workingday'] == 1)].groupby('hour').mean()[
    'count']

# In[77]:

all_no_dummy_interpolate[
    (all_no_dummy_interpolate['train'] == 1) & (all_no_dummy_interpolate['workingday'] == 0)].groupby('hour').mean()[
    'count']
...
train_no_dummy_year_temp_piv = all_no_dummy_interpolate[all_no_dummy_interpolate['train'] == 1].round().pivot_table(
    values='count', index='temp', columns=['workingday']).dropna()
...
testData_no_dummy = all_no_dummy_interpolate[
    (all_no_dummy_interpolate['day'] > 15) & (all_no_dummy_interpolate['day'] < 20)]
...
spark_Kaggle_test_dummy = sqlContext.createDataFrame(
    all_dummy_interpolate[all_dummy_interpolate['train'] != 1].reset_index())
spark_Kaggle_test_no_dummy = sqlContext.createDataFrame(
    all_no_dummy_interpolate[all_no_dummy_interpolate['train'] != 1].reset_index())
...
columns_to_hide = added_cat_var + dependent_variables + dependent_variables_log + dependent_variables_bc + dependent_variables_cat + [
    'train', 'index', 'hour_cat']
...
DUMMY = True
trainingData_dummy = sqlContext.createDataFrame(spark_train_dummy.rdd.map(transformToLabeledPoint),
                                                ["hour_cat", "label_count", "label_registered", "label_casual",
                                                 "features_to_filter"])
testData_dummy = sqlContext.createDataFrame(spark_test_dummy.rdd.map(transformToLabeledPoint),
                                            ["hour_cat", "label_count", "label_registered", "label_casual",
                                             "features_to_filter"])
testKaggle_dummy = sqlContext.createDataFrame(spark_Kaggle_test_dummy.rdd.map(transformToLabeledPointKaggle),
                                              ["hour_cat", "label_count", "label_registered", "label_casual",
                                               "features_to_filter"])
chiSq_dummy = sqlContext.createDataFrame(
    spark_train_dummy.unionAll(spark_test_dummy).rdd.map(transformToLabeledPointChiSqSelector),
    ["hour_cat", "label_count_cat", "label_registered_cat", "label_casual_cat", "features_to_filter"])
...
DUMMY = False
trainingData_no_dummy = sqlContext.createDataFrame(spark_train_no_dummy.rdd.map(transformToLabeledPoint),
                                                   ["hour_cat", "label_count", "label_registered", "label_casual",
                                                    "features_to_filter"])
testData_no_dummy = sqlContext.createDataFrame(spark_test_no_dummy.rdd.map(transformToLabeledPoint),
                                               ["hour_cat", "label_count", "label_registered", "label_casual",
                                                "features_to_filter"])
testKaggle_no_dummy = sqlContext.createDataFrame(spark_Kaggle_test_no_dummy.rdd.map(transformToLabeledPointKaggle),
                                                 ["hour_cat", "label_count", "label_registered", "label_casual",
                                                  "features_to_filter"])
chiSq_no_dummy = sqlContext.createDataFrame(
    spark_train_no_dummy.unionAll(spark_test_no_dummy).rdd.map(transformToLabeledPointChiSqSelector),
    ["hour_cat", "label_count_cat", "label_registered_cat", "label_casual_cat", "features_to_filter"])
...
featureIndexer = VectorIndexer(inputCol="features_to_filter", outputCol="indexedFeatures", maxCategories=4).fit(
    trainingData_no_dummy.unionAll(testData_no_dummy).unionAll(testKaggle_no_dummy))
...
def evaluate_prediction(predictionsTestData_r, predictionsTestData_c, predictionsTestData_count, lambdas, rounded=True):
    ...
    prediction_dict = {
        'label_registered': undo_bc_transform(pandas_pred_r.drop(['features'], axis=1).copy(), 'label_registered',
                                              lmbd=lambdas['registered']),
        'label_casual': undo_bc_transform(pandas_pred_c.drop(['features'], axis=1).copy(), 'label_casual',
                                          lmbd=lambdas['casual']),
        'label_sum': pandas_pred_sum.copy(),
        'label_count': undo_bc_transform(pandas_pred_count.drop(['features'], axis=1).copy(), 'label_count',
                                         lmbd=lambdas['count']),
        'rmsle_r': rmsle_r,
        'rmsle_c': rmsle_c,
        'rmsle_sum': rmsle_sum,
...
def evaluate_mixed_prediction(rf_all_sum, rf_all_count, gbt_all_sum, gbt_all_count, ratio=0.5, rounded=True):
    ...
    mixed_pandas_sum = ratio * rf_all_sum[['label_count', 'prediction']] + (1.0 - ratio) * gbt_all_sum[
        ['label_count', 'prediction']]
    ...
    mixed_pandas_count = ratio * rf_all_count[['label_count', 'prediction']] + (1.0 - ratio) * gbt_all_count[
        ['label_count', 'prediction']]
...
def predict_Kaggle_test(rf_predictionsTestData_r, rf_predictionsTestData_c, gbtr_predictionsTestData_r,
                        gbtr_predictionsTestData_c, lambdas, ratio=0.5):
...
def predict_count_Kaggle_test(rf_predictionsTestData_c, gbtr_predictionsTestData_c, lambdas, ratio=0.5):
    ...
    pandas_pred_sum['count'] = 0.0
    pandas_pred_sum['count'] = ratio * (rf_pandas_pred_c['prediction']) + (1.0 - ratio) * (
        gbtr_pandas_pred_c['prediction'])
    pandas_pred_sum['count'] = np.round(pandas_pred_sum['count']).astype(int)
    # other option to truncate floating point number instead of round
    # pandas_pred_sum['count'] = pandas_pred_sum['count'].astype(int)
...
def concatenate_predictions(pred_dict):
    ...
    return all_registered.reset_index(drop=True), all_casual.reset_index(drop=True), all_sum.reset_index(
        drop=True), all_count.reset_index(drop=True)
...
for hour_cat in hour_cat_list:
    ...
    plt.plot(range(1, len(no_dummy_cols) + 1), rmsle_hour_cat[hour_cat]['sum'], label='Sum', c='red', marker='o',
...
feat_num_hour_cat = {}
for hour_cat in hour_cat_list:
    feat_num_hour_cat[hour_cat] = {'label_registered': rmsle_hour_cat[hour_cat]['registered'][:max_num_features].index(
        min(rmsle_hour_cat[hour_cat]['registered'][:max_num_features])) + 1,
                                   'label_casual': rmsle_hour_cat[hour_cat]['casual'][:max_num_features] \
                                                       .index(
                                       min(rmsle_hour_cat[hour_cat]['casual'][:max_num_features])) + 1,
                                   'label_count': rmsle_hour_cat[hour_cat]['count'][:max_num_features] \
                                                      .index(
                                       min(rmsle_hour_cat[hour_cat]['count'][:max_num_features])) + 1}

# In[126]:

for hour_cat in hour_cat_list:
    for label in ['label_registered', 'label_casual', 'label_count']:
        if (feat_num_hour_cat[hour_cat][label] < min_num_features): feat_num_hour_cat[hour_cat][
            label] = min_num_features
...
for hour_cat in hour_cat_list:
    ...
    for label in ["label_count", "label_registered", "label_casual"]:
        ...
        selector_no_dummy = ChiSqSelector(selectorType="numTopFeatures", fpr=0.05,
                                          numTopFeatures=feat_num_hour_cat[hour_cat][label],
                                          featuresCol="indexedFeatures", outputCol="features", labelCol=label + "_cat")

        selected_features_no_dummy = selector_no_dummy.fit(
            featureIndexer.transform(chiSq_no_dummy.filter(chiSq_no_dummy.hour_cat == hour_cat)))
        ...
        print(label + "_no_dummy, cat_" + str(
            hour_cat) + ": ChiSqSelector output with %d features selected below %.2f p-Value" % (
...
for hour_cat in hour_cat_list:
    ...
    print("==========================================")
    dt_predictionsTestData_r, dt_predictionsTestData_c, dt_predictionsTestData_count = make_prediction(
        dt_cv_models_dict[hour_cat], feature_selector_no_dummy[hour_cat], hour_cat, dummy=False)
    dt_pred_dict[hour_cat] = evaluate_prediction(dt_predictionsTestData_r, dt_predictionsTestData_c,
                                                 dt_predictionsTestData_count, lambdas=bc_lambda_dict[hour_cat])
...
for hour_cat in hour_cat_list:
    ...
    for label in ['label_registered', 'label_casual', 'label_count']:
        ...
        paramGrid = ParamGridBuilder().addGrid(lr.regParam, regParam).addGrid(lr.elasticNetParam,
                                                                              elasticNetParam).addGrid(lr.labelCol,
                                                                                                       [label]).build()
        ...
        cvModel = crossval.fit(feature_selector_dummy[hour_cat][label].transform(
            trainingData_dummy.filter(trainingData_dummy.hour_cat == hour_cat).unionAll(
                testData_dummy.filter(testData_dummy.hour_cat == hour_cat))))
        end = time.time()
...
for hour_cat in hour_cat_list:
    ...
    print("Category: " + str(hour_cat))
    print("==========================================")
    lr_predictionsTestData_r, lr_predictionsTestData_c, lr_predictionsTestData_count = make_prediction(
        lr_cv_models_dict[hour_cat], feature_selector_dummy[hour_cat], hour_cat, dummy=True)
    lr_pred_dict[hour_cat] = evaluate_prediction(lr_predictionsTestData_r, lr_predictionsTestData_c,
                                                 lr_predictionsTestData_count, lambdas=bc_lambda_dict[hour_cat])
...
for hour_cat in hour_cat_list:
    ...
    for label in ['label_registered', 'label_casual', 'label_count']:
        ...
        paramGrid = ParamGridBuilder().addGrid(rf.numTrees, numTrees).addGrid(rf.maxDepth, maxDepth).addGrid(
            rf.minInstancesPerNode, minInstancesPerNode).addGrid(rf.maxBins, maxBins).addGrid(rf.labelCol,
                                                                                              [label]).build()
        ...
        start = time.time()
        cvModel = crossval.fit(feature_selector_no_dummy[hour_cat][label].transform(featureIndexer.transform(
            trainingData_no_dummy.filter(trainingData_no_dummy.hour_cat == hour_cat).unionAll(
                testData_no_dummy.filter(testData_no_dummy.hour_cat == hour_cat)))))
        print("======= CV for hour_cat " + str(hour_cat) + " and " + label + " =========")
        end = time.time()
        print("Time taken to train model with k-fold cv: " + str(end - start) + " seconds")
...
for hour_cat in hour_cat_list:
    ...
    print("Category: " + str(hour_cat))
    print("==========================================")
    rf_predictionsTestData_r, rf_predictionsTestData_c, rf_predictionsTestData_count = make_prediction(
        rf_cv_models_dict[hour_cat], feature_selector_no_dummy[hour_cat], hour_cat, dummy=False)
    rf_pred_dict[hour_cat] = evaluate_prediction(rf_predictionsTestData_r, rf_predictionsTestData_c,
                                                 rf_predictionsTestData_count, lambdas=bc_lambda_dict[hour_cat])
...
for hour_cat in hour_cat_list:
    ...
    for label in ['label_registered', 'label_casual', 'label_count']:
        ...
        paramGrid = ParamGridBuilder().addGrid(gbt.minInstancesPerNode, minInstancesPerNode).addGrid(gbt.maxDepth,
                                                                                                     maxDepth).addGrid(
            gbt.maxIter, maxIter).addGrid(gbt.maxBins, maxBins).addGrid(gbt.labelCol, [label]).build()
...
for hour_cat in hour_cat_list:
    ...
    print("Category: " + str(hour_cat))
    print("==========================================")
    gbt_predictionsTestData_r, gbt_predictionsTestData_c, gbt_predictionsTestData_count = make_prediction(
        gbt_cv_models_dict[hour_cat], feature_selector_no_dummy[hour_cat], hour_cat, dummy=False)
    gbt_pred_dict[hour_cat] = evaluate_prediction(gbt_predictionsTestData_r, gbt_predictionsTestData_c,
                                                  gbt_predictionsTestData_count, lambdas=bc_lambda_dict[hour_cat])
...
for hour_cat in hour_cat_list:
    sns.distplot(gbt_all_registered[gbt_all_registered['hour_cat'] == hour_cat]['label_registered'], bins=24,
                 ax=axes[hour_cat][0], axlabel='HOUR_CAT ' + str(hour_cat) + ', registered', color='orange')
    sns.distplot(gbt_all_registered[gbt_all_registered['hour_cat'] == hour_cat]['prediction'], bins=24,
                 ax=axes[hour_cat][1], color='green')
    plt.subplots_adjust(hspace=0.3)
    plt.tight_layout
...
fig, axes = plt.subplots(nrows=8, ncols=2, figsize=(11, 19))
for hour_cat in hour_cat_list:
    sns.distplot(gbt_all_registered[gbt_all_registered['hour_cat'] == hour_cat]['label_casual'], bins=24,
                 ax=axes[hour_cat][0], axlabel='HOUR_CAT ' + str(hour_cat) + ', registered', color='red')
    sns.distplot(gbt_all_registered[gbt_all_registered['hour_cat'] == hour_cat]['prediction'], bins=24,
                 ax=axes[hour_cat][1], color='green')
    plt.subplots_adjust(hspace=0.3)
    plt.tight_layout
...
fig, axes = plt.subplots(nrows=8, ncols=2, figsize=(11, 19))
for hour_cat in hour_cat_list:
    sns.distplot(gbt_all_registered[gbt_all_registered['hour_cat'] == hour_cat]['label_count'], bins=24,
                 ax=axes[hour_cat][0], axlabel='HOUR_CAT ' + str(hour_cat) + ', registered', color='blue')
    sns.distplot(gbt_all_registered[gbt_all_registered['hour_cat'] == hour_cat]['prediction'], bins=24,
                 ax=axes[hour_cat][1], color='green')
    plt.subplots_adjust(hspace=0.3)
    plt.tight_layout
...
for ratio in ratio_array:
    rmsle_sum_rf_gbt.append(
        evaluate_mixed_prediction(rf_all_sum, rf_all_count, gbt_all_sum, gbt_all_count, ratio=float(ratio))[
            'rmsle_sum'])
    rmsle_count_rf_gbt.append(
        evaluate_mixed_prediction(rf_all_sum, rf_all_count, gbt_all_sum, gbt_all_count, ratio=float(ratio))[
            'rmsle_count'])
    rmsle_sum_lr_rf.append(
        evaluate_mixed_prediction(lr_all_sum, lr_all_count, rf_all_sum, rf_all_count, ratio=float(ratio))['rmsle_sum'])
    rmsle_count_lr_rf.append(
        evaluate_mixed_prediction(lr_all_sum, lr_all_count, rf_all_sum, rf_all_count, ratio=float(ratio))[
            'rmsle_count'])
    rmsle_sum_lr_gbt.append(
        evaluate_mixed_prediction(lr_all_sum, lr_all_count, gbt_all_sum, gbt_all_count, ratio=float(ratio))[
            'rmsle_sum'])
    rmsle_count_lr_gbt.append(
        evaluate_mixed_prediction(lr_all_sum, lr_all_count, gbt_all_sum, gbt_all_count, ratio=float(ratio))[
            'rmsle_count'])
...
for hour_cat in hour_cat_list:
    rf_predictionsTestData_r, rf_predictionsTestData_c, rf_predictionsTestData_count = make_prediction(
        rf_cv_models_dict[hour_cat], feature_selector_no_dummy[hour_cat], hour_cat, dummy=False, Kaggle=True)
    lr_predictionsTestData_r, lr_predictionsTestData_c, lr_predictionsTestData_count = make_prediction(
        lr_cv_models_dict[hour_cat], feature_selector_dummy[hour_cat], hour_cat, dummy=True, Kaggle=True)
    predicted_Kaggle_sum_cat[hour_cat] = predict_Kaggle_test(rf_predictionsTestData_r, rf_predictionsTestData_c,
                                                             lr_predictionsTestData_r, lr_predictionsTestData_c,
                                                             lambdas=bc_lambda_dict[hour_cat], ratio=float(0.2))
    predicted_Kaggle_count_cat[hour_cat] = predict_count_Kaggle_test(rf_predictionsTestData_count,
                                                                     lr_predictionsTestData_count,
                                                                     lambdas=bc_lambda_dict[hour_cat], ratio=float(0.2))
